...
function EncodeDeepString(const S: AnsiString): TBytes;
var
  UB: TBytes;
  I: Integer;
begin
...
function DecodeDeepString(const Data: TBytes): AnsiString;
var
  UB: TBytes;
  I: Integer;
begin
...
function SerializeDeepRoot(const Root: TDeepRoot): TBytes;
var
  L10, L9, L8, L7, L6, L5, L4, L3, L2, L1, RootEnc: TBytes;
begin
...
function DeserializeDeepRoot(const Data: TBytes): TDeepRoot;
var
  L1, L2, L3, L4, L5, L6, L7, L8, L9, L10: TBytes;
begin
  L1 := DecodeBytes(Data);
  L2 := DecodeBytes(L1);
  L3 := DecodeBytes(L2);
...
begin
  try
    ...
    DB := TGarryDatabase.Create('test_delphi.db', True);
    WriteLn('  OK');
    ...
    DB := TGarryDatabase.Create('test_delphi.db', False);
    Value := DB.Get('a');
