...
static void lz77_match_find(const char *src, size_t src_len, size_t ip,
                            int *hash_table, int min_match,
                            size_t max_match, size_t *match_off,
                            size_t *match_len)
{
    unsigned int h;
    int idx;
    ...
    *match_len = 0;
    ...
    if (ip + min_match > src_len) return;
    ...
    while (s < ip && best_len < limit) {
        size_t len = 0;
        size_t max_cmp = ip - s;
        if (max_cmp > limit) max_cmp = limit;
...
char *lz4_compress(const char *src, size_t src_len, size_t *out_len)
{
    ...
    while (ip < ip_limit) {
        lz77_match_find(src, src_len, ip, hash_table, MIN_MATCH,
                        MAX_MATCH, &match_off, &match_len);
                     ...
                     size_t dst_capacity, size_t *out_len)
{
    ...
    while (ip < ip_limit) {
        ...
        offset = (size_t)lz4_byte_to_int(src[ip]) +
                 (size_t)lz4_byte_to_int(src[ip + 1]) * 256;
        ip += 2;
