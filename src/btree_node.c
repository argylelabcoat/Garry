        ...
        for (j = 1; j < rec_count; j++)
        {
            if (entry_idx < GARRY_MAX_KEYS_PER_NODE)
            {
                ...
                if (rlen > 0)
                {
                    ...
                    if (!garry_decode_kv(rec_buf, rlen, ckey, &ckey_len, node->values[entry_idx],
                                         &vlen_out))
                    {
                        continue;
                    }
...
void garry_store_node(garry_buffer_pool *pool, garry_i32 pid, garry_btree_node *node)
{
    ...
    for (i = 0; i < node->entry_count; i++)
    {
        ...
        {
            rec_len =
                garry_encode_kv(node->keys[i], node->key_lens[i], node->values[i], 0, rec_buf);
        }
        else
        {
            rec_len = garry_encode_kv(node->keys[i], node->key_lens[i], node->values[i],
                                      node->value_lens[i], rec_buf);
        }
        if (rec_len > 0)
        {
            for (j = 0; j < rec_len; j++)
